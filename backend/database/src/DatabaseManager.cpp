...
DatabaseResult DatabaseManager::initialize(const std::string& dbPath,
    ...
    if (encryptionKey.length() < 32) {
        return DatabaseResult(false, "Encryption key must be at least 32 characters long",
                              SQLITE_MISUSE);
    }
    ...
    auto tryInitWithSettings = [&](bool useDefaultSettings) -> DatabaseResult {
        ...
        if (!useDefaultSettings) {
            const char* cipherPragmas[] = {"PRAGMA cipher_page_size = 4096;",
    ...
    if (!res && fileExists) {
        std::cout << "Standard initialization failed, attempting fallback to default SQLCipher "
                     "settings..."
                  << std::endl;
...
DatabaseResult DatabaseManager::executeQuery(const std::string& sql,
    ...
    if (containsDangerousSQL(sql)) {
        return DatabaseResult(false, "Query contains potentially dangerous SQL patterns",
                              SQLITE_MISUSE);
    }
    ...
    if (callback) {
        try {
            ...
            return DatabaseResult(false, "Callback execution failed: " + std::string(e.what()),
                                  SQLITE_ERROR);
        }
    }
...
DatabaseResult DatabaseManager::executeQuery(const std::string& sql,
    ...
    if (static_cast<int>(params.size()) != expectedParams) {
        ...
        std::string error = "Parameter count mismatch: expected " + std::to_string(expectedParams) +
    ...
    for (size_t i = 0; i < params.size(); ++i) {
        if (params[i].length() > 1048576) {
            sqlite3_finalize(stmt);
            return DatabaseResult(false, "Parameter " + std::to_string(i + 1) + " too large",
                                  SQLITE_MISUSE);
        }

        result = sqlite3_bind_text(stmt, static_cast<int>(i + 1), params[i].c_str(), -1,
                                   SQLITE_TRANSIENT);
        if (result != SQLITE_OK) {
            sqlite3_finalize(stmt);
...
DatabaseResult DatabaseManager::setSchemaVersion(int version) {
    ...
    if (result != SQLITE_OK)
        return DatabaseResult(false, "Failed to prepare version update", result);
    ...
    if (result != SQLITE_DONE)
        return DatabaseResult(false, "Failed to update schema version", result);
...
DatabaseResult DatabaseManager::createBackup(const std::string& backupPath) {
    ...
    if (result != SQLITE_DONE)
        return DatabaseResult(false, "Backup failed", result);
...
DatabaseResult DatabaseManager::verifyIntegrity() {
    ...
    if (rc != SQLITE_OK)
        return DatabaseResult(false, "Failed to prepare integrity check", rc);
...
DatabaseResult DatabaseManager::createInitialSchema() {
    ...
    if (rc != SQLITE_OK)
        return DatabaseResult(false, "Failed to create version table", rc);
...
DatabaseResult DatabaseManager::validateEncryption() {
    sqlite3_stmt* stmt = nullptr;
    int result =
        sqlite3_prepare_v2(m_db, "SELECT COUNT(*) FROM sqlite_master;", -1, &stmt, nullptr);

    if (result != SQLITE_OK) {
        return DatabaseResult(
            false, "Encryption validation failed: " + std::string(sqlite3_errmsg(m_db)), result);
    }
...
DatabaseResult DatabaseManager::setupPragmas() {
    const char* pragmas[] = {"PRAGMA foreign_keys = ON;", "PRAGMA secure_delete = ON;",
...
DatabaseResult DatabaseManager::checkConnectionHealth() {
    ...
    if (result != SQLITE_OK) {
        ...
        if (m_connectionAttempts > 3)
            return DatabaseResult(false, "Connection unhealthy", SQLITE_ERROR);
...
DatabaseResult DatabaseManager::verifyBackupIntegrity(sqlite3* backupDb) {
    if (!backupDb)
        return DatabaseResult(false, "Backup handle is null", SQLITE_MISUSE);
    return DatabaseResult(true, "Integrity verified");
}
