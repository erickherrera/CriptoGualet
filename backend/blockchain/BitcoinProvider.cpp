...
class BlockCypherProvider : public BitcoinProvider {
  public:
    ...
    std::optional<AddressInfo> getAddressInfo(const std::string& address, uint32_t limit) override {
        if (!m_client) {
            return std::nullopt;
        }
        ...
        if (txs) {
            ...
            if (info.transactionCount == 0) {
                info.transactionCount = static_cast<uint32_t>(info.recentTransactions.size());
            }
        }
...
class BitcoinRpcProvider : public BitcoinProvider {
  public:
    explicit BitcoinRpcProvider(const ProviderConfig& config)
        : m_rpcUrl(config.rpcUrl),
          m_rpcUsername(config.rpcUsername),
    ...
    std::optional<AddressInfo> getAddressInfo(const std::string& address, uint32_t limit) override {
        ...
        if (confirmedResult) {
            info.confirmedBalance = btcToSatoshis(parseBtcValue(*confirmedResult));
        }

        if (totalResult) {
            uint64_t totalBalance = btcToSatoshis(parseBtcValue(*totalResult));
            if (totalBalance >= info.confirmedBalance) {
                info.unconfirmedBalance = totalBalance - info.confirmedBalance;
            }
        }

        auto txsResult = call("listtransactions", {"*", static_cast<int>(limit), 0, true});
  ...
  private:
    std::optional<json> call(const std::string& method, json params) {
        ...
        json payload = {
            {"jsonrpc", "1.0"}, {"id", "criptogualet"}, {"method", method}, {"params", params}};
        ...
        if (!m_rpcUsername.empty()) {
            response =
                cpr::Post(cpr::Url{m_rpcUrl}, cpr::Body{payload.dump()}, headers,
                          cpr::Authentication{m_rpcUsername, m_rpcPassword, cpr::AuthMode::BASIC},
                          cpr::Timeout{10000});
        } else {
...
std::unique_ptr<BitcoinProvider> CreateProvider(const ProviderConfig& config) {
    switch (config.type) {
        case ProviderType::BitcoinRpc:
            if (!config.allowInsecureHttp && config.rpcUrl.rfind("http://", 0) == 0) {
                return nullptr;
            }
