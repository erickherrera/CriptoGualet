...
Result<std::optional<std::string>> SettingsRepository::getUserSetting(int userId,
    ...
    if (rc != SQLITE_OK) {
        return Result<std::optional<std::string>>("Failed to prepare settings query", 500);
    }
    ...
    return Result<std::optional<std::string>>("Database error while retrieving setting", 500);
}
...
Result<std::map<std::string, std::string>> SettingsRepository::getUserSettings(
    ...
    int rc = sqlite3_prepare_v2(m_dbManager.getHandle(), sql.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        return Result<std::map<std::string, std::string>>("Failed to prepare settings query", 500);
    }
    ...
    return Result<std::map<std::string, std::string>>("Database error while retrieving settings",
                                                      500);
}

Result<bool> SettingsRepository::setUserSetting(int userId, const std::string& key,
...
Result<bool> SettingsRepository::deleteUserSetting(int userId, const std::string& key) {
    REPO_SCOPED_LOG(COMPONENT_NAME, "deleteUserSetting");

    const std::string sql = "DELETE FROM user_settings WHERE user_id = ? AND setting_key = ?";
