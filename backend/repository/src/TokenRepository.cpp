...
TokenRepository::TokenRepository(Database::DatabaseManager& dbManager) : m_dbManager(dbManager) {
}

Result<Token> TokenRepository::createToken(int wallet_id, const std::string& contract_address,
                                           const std::string& symbol, const std::string& name,
                                           int decimals) {
    std::string sql =
        "INSERT INTO erc20_tokens (wallet_id, contract_address, symbol, name, decimals) VALUES (?, "
        "?, ?, ?, ?);";

    auto result = m_dbManager.executeQuery(
        sql, {std::to_string(wallet_id), contract_address, symbol, name, std::to_string(decimals)});
...
Result<Token> TokenRepository::getToken(int wallet_id, const std::string& contract_address) {
    std::string sql =
        "SELECT id, wallet_id, contract_address, symbol, name, decimals, created_at FROM "
        "erc20_tokens WHERE wallet_id = ? AND contract_address = ?;";

    Result<Token> repoResult;
    repoResult.success = false;

    auto dbResult = m_dbManager.executeQuery(
        sql, {std::to_string(wallet_id), contract_address}, [&](sqlite3* db) {
            sqlite3_stmt* stmt;
