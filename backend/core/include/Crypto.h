...
// BIP44 defines the hierarchical structure: m / purpose' / coin_type' / account' / change /
// address_index For Bitcoin mainnet: m/44'/0'/0'/0/0 (first receiving address) For Bitcoin testnet:
// m/44'/1'/0'/0/0
...
std::string GenerateTOTP(const std::vector<uint8_t>& secret,
                         ...
                         uint32_t timestep = 30, uint32_t digits = 6);
...
bool VerifyTOTP(const std::vector<uint8_t>& secret, const std::string& code, uint32_t window = 1,
                uint32_t timestep = 30, uint32_t digits = 6);
