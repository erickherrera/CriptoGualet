...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    // Test 1: RLP encoding (Ethereum transactions)
    {
        ...
        std::string hexStr =
            RLP::Encoder::BytesToHex(std::vector<uint8_t>(data, data + std::min(size, size_t(32))));
        ...
        for (size_t i = 0; i < 5 && (i * 10) < size; i++) {
            items.push_back(
                std::vector<uint8_t>(data + i * 10, data + std::min(i * 10 + 10, size)));
        }
        if (!items.empty()) {
    ...
    // Test 2: Bitcoin script parsing
    {
        ...
        for (size_t scriptSize : {size_t(0), size_t(1), size_t(19), size_t(20), size_t(21),
                                  size_t(32), size_t(33), size_t(40), size_t(100)}) {
            if (scriptSize <= size) {
    ...
    // Test 3: Transaction hash parsing
    {
        ...
        if (size >= 32) {
            ...
            std::string hexStr = RLP::Encoder::BytesToHex(
                std::vector<uint8_t>(data, data + std::min(size, size_t(32))));
            (void)hexStr;
    ...
    // Test 4: Bitcoin transaction structure fuzzing
    {
        ...
        for (size_t i = 0; i < numInputs && (10 + i * 32) < size; i++) {
            ...
            input.txid = RLP::Encoder::BytesToHex(
                std::vector<uint8_t>(data + 10 + i * 32, data + std::min(10 + i * 32 + 32, size)));
        ...
        for (size_t i = 0; i < numOutputs && (20 + i * 20) < size; i++) {
            ...
            if (scriptLen > 0) {
                output.script_pubkey = RLP::Encoder::BytesToHex(
                    std::vector<uint8_t>(data + scriptStart, data + scriptStart + scriptLen));
            }
        ...
        if (!tx.inputs.empty() && !tx.outputs.empty()) {
            ...
            std::string prevScriptPubkey =
                "0014751e76e8199196d454941c45d1b3a323f1433bd6";  // Dummy P2WPKH
