...
void testRBFTransactionCreation() {
    ...
    std::cout << "    Created RBF-enabled transaction with " << tx.inputs.size() << " inputs" << std::endl;
    std::cout << "    All inputs have sequence: 0x" << std::hex << Crypto::TransactionInput::SEQUENCE_RBF_ENABLED << std::dec << std::endl;
...
void testRBFFeeBumpingRequirements() {
    ...
    std::cout << "    Original fee: " << original_fee << " satoshis @ " << original_fee_rate << " sat/byte" << std::endl;
    std::cout << "    New fee: " << new_fee << " satoshis @ " << new_fee_rate << " sat/byte" << std::endl;
...
void testCPFPTransactionChaining() {
    ...
    std::cout << "    Parent size: " << parent_size << " bytes, fee: " << parent_fee << " sat" << std::endl;
...
void testRBFSequenceNumberEdgeCases() {
    ...
    std::cout << "    RBF sequence (0xFFFFFFFD): " << (rbf_signaled ? "RBF enabled" : "RBF disabled") << std::endl;
