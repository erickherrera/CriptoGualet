...
class SessionTestHelpers {
  public:
    static UserSession createTestSession(
        int userId = SessionTestConfig::TEST_USER_ID,
    ...
    static bool isValidSessionIdFormat(const std::string& sessionId) {
        return sessionId.length() == SessionTestConfig::SESSION_ID_LENGTH &&
               std::all_of(sessionId.begin(), sessionId.end(), [](char c) {
...
class MockSessionRepository {
  ...
  public:
    ...
    void cleanupExpiredSessions() {
        ...
        storedSessions_.erase(std::remove_if(storedSessions_.begin(), storedSessions_.end(),
                                             [](const MockSessionRecord& s) {
                                                 return !s.isActive;
                                             }),
                              storedSessions_.end());
    }
...
void testCreateSessionWithValidData() {
    ...
    bool passed =
        !sessionId.empty() && sessionId.length() == SessionTestConfig::SESSION_ID_LENGTH &&
        SessionTestHelpers::isValidSessionIdFormat(sessionId) && manager.validateSession(sessionId);
...
void testSessionDataInitialization() {
    ...
    bool passed = session && session->userId == 2 && session->username == "anothertestuser" &&
                  !session->walletData.btcAddress.empty() &&
                  !session->walletData.ltcAddress.empty() &&
                  !session->walletData.ethAddress.empty() && session->walletData.btcBalance > 0.0;
...
void testSessionIdUniqueness() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (generatedIds.count(sessionId) > 0) {
            allUnique = false;
            std::cout << COLOR_RED << "  Duplicate session ID generated" << COLOR_RESET
                      << std::endl;
            break;
        }
...
void testSessionSensitiveDataWipe() {
    ...
    bool hasDataBefore =
        !session->walletData.btcAddress.empty() && session->walletData.btcBalance > 0.0;
    ...
    bool dataWiped =
        session->walletData.btcAddress.empty() && session->walletData.ltcAddress.empty() &&
        session->walletData.ethAddress.empty() && session->walletData.btcBalance == 0.0 &&
        session->walletData.ltcBalance == 0.0 && session->walletData.ethBalance == 0.0;
...
void testSessionExpiration() {
    ...
    testSession.createdAt =
        std::chrono::system_clock::now() - std::chrono::minutes(30);  // Created 30 min ago
    testSession.expiresAt =
        std::chrono::system_clock::now() - std::chrono::minutes(15);  // Expires in 15 min
    testSession.isActive = true;
...
void testDatabasePerformance() {
    ...
    std::cout << "  Retrieved " << retrievedCount << "/" << testCount << " sessions in "
...
void testSessionIdSecurity() {
    ...
    TEST_ASSERT(validFormat && sequentialFormat && !randomFormat,
                "Session ID format security should be enforced");
...
void testSessionTimeoutSecurity() {
    ...
    TEST_ASSERT(timeoutValidationSecure && expiredSessionsStayExpired && timeoutCannotBeManipulated,
                "Session timeout security should work correctly");
...
void testAuthenticationBypassProtection() {
    ...
    TEST_ASSERT(
        unauthenticatedSessionCreationBlocked && totpBypassBlocked && sessionHijackingPrevented,
        "Authentication bypass protection should work correctly");
...
void testConcurrentSessionSecurity() {
    ...
    TEST_ASSERT(sessionIsolationWorks && crossSessionDataLeakagePrevented,
                "Concurrent session security should work correctly");
...
void testDataEncryption() {
    ...
    TEST_ASSERT(walletDataEncrypted && personalInfoEncrypted && transactionDataEncrypted,
...
void testPrivilegeEscalation() {
    ...
    TEST_ASSERT(privilegeEscalationBlocked && adminAccessWithoutAuth && sensitiveAccessWithoutTOTP,
                "Privilege escalation protection should work correctly");
...
void testSessionInvalidationSecurity() {
    ...
    TEST_ASSERT(
        secureInvalidation && accidentalInvalidationPrevented && maliciousInvalidationBlocked,
        "Session invalidation security should work correctly");
...
void testLoggingAndAuditing() {
    ...
    TEST_ASSERT(sessionCreationLogged && sessionAccessLogged && invalidationLogged &&
                    suspiciousActivityLogged,
                "Session logging and auditing should work correctly");

    if (sessionCreationLogged && sessionAccessLogged && invalidationLogged &&
        suspiciousActivityLogged) {
        TEST_PASS();
    } else {
...
void testMemorySecurity() {
    ...
    TEST_ASSERT(memoryClearedOnDestruction && sensitiveDataZeroed && bufferOverflowPrevented,
                "Memory security should work correctly");
...
void testUserSessionInitialization() {
    ...
    bool passed = (session.userId == 1 && session.username == "testuser" &&
                   !session.sessionId.empty() && session.totpAuthenticated && session.isActive);
...
void testMultiSessionScenarios() {
    ...
    TEST_ASSERT(differentUsersHaveDifferentIds,
    ...
    bool passed =
        differentUsersHaveDifferentIds && sameUserHasMultipleSessions && sameUserSessionsAreUnique;
...
int main() {
    ...
    std::cout << COLOR_GREEN << "Tests Passed: " << TestGlobals::g_testsPassed << COLOR_RESET
              << std::endl;
    std::cout << COLOR_RED << "Tests Failed: " << TestGlobals::g_testsFailed << COLOR_RESET
              << std::endl;

    std::cout << COLOR_GREEN << "=== Consolidated Session Tests Completed ===" << COLOR_RESET
              << std::endl;
