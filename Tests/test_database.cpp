...
class DatabaseTester {
  ...
  public:
    DatabaseTester()
        : db(Database::DatabaseManager::getInstance()),
          ...
          backupPath(TestUtils::getWritableTestPath("test_criptogualet_backup.db")),
          testsPassed(true),
          testsRun(0) {
    }
    ...
    bool runAllTests() {
        ...
        try {
            ...
            if (!safeTestRun(&DatabaseTester::testDatabaseInitialization,
                ...
                return false;
            if (!safeTestRun(&DatabaseTester::testBasicOperations, "Basic Operations"))
                return false;
            if (!safeTestRun(&DatabaseTester::testTransactionManagement, "Transaction Management"))
                return false;
            if (!safeTestRun(&DatabaseTester::testSchemaVersioning, "Schema Versioning"))
                return false;
            ...
            if (ENABLE_BACKUP_TEST) {
                std::cout << "\n[NOTE] Starting backup test (may take 10-30 seconds)..."
                          << std::endl;
                if (!safeTestRun(&DatabaseTester::testBackupRestore, "Backup & Restore"))
                    return false;
            } else {
                std::cout << "\n[SKIPPED] Backup & Restore test (disabled)" << std::endl;
                std::cout << "   To enable: Set ENABLE_BACKUP_TEST = true in test_database.cpp"
                          << std::endl;
            }
            ...
            if (testsPassed) {
                ...
                std::cout << "[OK] Database infrastructure is working correctly" << std::endl;
                ...
                std::cout << "[FAILED] " << (testsRun - getPassedTests()) << " out of " << testsRun
  ...
  private:
    void testDatabaseInitialization() {
        ...
        std::cout << "   Initializing with key length: " << encryptionKey.length() << std::endl;
        ...
        if (initResult.success) {
            ...
            std::cout << "   [ERROR] Initialization failed: " << initResult.message << std::endl;
        }
    }
    ...
    void testBasicOperations() {
        ...
        std::vector<std::string> walletParams = {"Main Wallet",
                                                 "bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh"};
        auto insertResult =
            db.executeQuery("INSERT INTO wallets (name, address) VALUES (?, ?);", walletParams);
    ...
    void testTransactionManagement() {
        ...
        std::vector<std::string> params = {"Savings Wallet",
                                           "bc1qar0srrr7xfkvy5l643lydnw9re59gtzzwf5mdq"};
        auto transInsert =
            db.executeQuery("INSERT INTO wallets (name, address) VALUES (?, ?);", params);
    ...
    void testDataIntegrity() {
        ...
        auto fkTest = db.executeQuery(
            "INSERT INTO transactions (wallet_id, txid, "
        ...
        std::cout << "   [OK] SQLCipher encryption protecting data at rest" << std::endl;
    }
    ...
    void testBackupRestore() {
        ...
        if (backupResult.success) {
            ...
            std::cout << "   [WARNING] Backup creation failed: " << backupResult.message
                      << std::endl;
    ...
    bool safeTestRun(TestMethod method, const std::string& testName) {
        try {
            ...
            std::cout << "[FAILED] " << testName << " failed with unknown exception" << std::endl;
            testsPassed = false;
            return true;  // Continue with other tests
        }
    }
...
int main() {
    ...
    try {
        ...
        std::cout << "Tests completed with result: " << (result ? "PASS" : "FAIL") << std::endl;
