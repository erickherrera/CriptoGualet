...
void QtSeedDisplayDialog::setupUI() {
  ...
  m_qrLabel->setStyleSheet("border: 2px solid #007bff; background-color: "
  ...
  warningLabel->setStyleSheet(
      "background-color: #fff3cd; color: #856404; padding: 8px; border: 1px "
      "solid #ffeaa7; border-radius: 4px; font-size: 11px;");
  ...
  m_seedTextEdit->setStyleSheet(
      "background-color: #1e1e1e; color: white; border: 1px solid #555555; "
  ...
  m_confirmCheckbox->setStyleSheet(
      "QCheckBox { font-weight: bold; padding: 5px; }");
  ...
  m_confirmButton->setStyleSheet(
      ...
      "QPushButton:hover:enabled { background-color: #0056b3; } "
...
void QtSeedDisplayDialog::generateQRCode() {
  ...
  if (qrData.width > 0 && qrData.height > 0 && !qrData.data.empty()) {
    ...
    QImage scaledImage =
        qrImage.scaled(scaledSize, scaledSize, Qt::KeepAspectRatio,
                       Qt::FastTransformation // Use fast transformation for
    ...
    for (int y = 0; y < scaledSize; ++y) {
      for (int x = 0; x < scaledSize; ++x) {
        paddedImage.setPixelColor(x + padding, y + padding,
    ...
    if (!qrSuccess) {
      ...
      m_qrLabel->setStyleSheet(m_qrLabel->styleSheet() +
