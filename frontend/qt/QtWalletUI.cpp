...
QtWalletUI::QtWalletUI(QWidget *parent)
    : QWidget(parent), m_themeManager(nullptr), m_centeringLayout(nullptr),
      m_leftSpacer(nullptr), m_rightSpacer(nullptr), m_headerSection(nullptr),
      m_headerTitle(nullptr), m_balanceTitle(nullptr), m_balanceLabel(nullptr),
      m_toggleBalanceButton(nullptr), m_refreshButton(nullptr),
      m_bitcoinWalletCard(nullptr), m_litecoinWalletCard(nullptr),
  ...
  QTimer::singleShot(100, this, [self]() {
    ...
    QObject::connect(self->m_priceUpdateTimer, &QTimer::timeout, self.data(),
  ...
  if (m_themeManager) {
    QObject::connect(m_themeManager, &QtThemeManager::themeChanged, this,
...
void QtWalletUI::setupUI() {
  ...
  m_contentLayout->setContentsMargins(contentMargin, contentMargin,
  ...
  // Optimized bottom spacer - reduced from 20,40 to 12,24 for better space
  // utilization
  m_contentLayout->addItem(
      new QSpacerItem(12, 24, QSizePolicy::Minimum, QSizePolicy::Expanding));
  ...
  // Initialize responsive layout after UI setup - add safety delay and null
  // checks
  QPointer<QtWalletUI> self(this);
  QTimer::singleShot(100, this, [self]() {
    ...
    if (self && self->m_mainLayout && self->m_contentLayout) {
      ...
      qDebug() << "QtWalletUI: Skipping layout updates due to null pointers or "
                  "object destruction";
    }
  });
}
...
void QtWalletUI::createHeaderSection() {
  ...
  m_toggleBalanceButton->setFixedSize(
      28, 28); // Reduced from 32,32 for better scale
  ...
  m_toggleBalanceButton->setAccessibleDescription(
      "Press to toggle balance visibility");
  ...
  m_refreshButton->setAccessibleName(
      "Refresh Balances"); // PHASE 3: Screen reader support
...
void QtWalletUI::createActionButtons() {
  ...
  if (m_wallet) {
    ...
    m_bitcoinWalletCard->setTransactionHistory(
        "No transactions yet.<br><br>Transaction history will be displayed "
        "here.");
  ...
  if (m_litecoinWallet) {
    ...
    connect(m_litecoinWalletCard, &QtExpandableWalletCard::receiveRequested,
  ...
  if (m_ethereumWallet) {
    ...
    connect(m_ethereumWalletCard, &QtExpandableWalletCard::receiveRequested,
    ...
    m_importTokenButton->setToolTip(
        "Import any ERC20 token by contract address to your Ethereum wallet.");
    m_importTokenButton->setCursor(Qt::PointingHandCursor);
    m_importTokenButton->setObjectName("importTokenButton");

    connect(m_importTokenButton, &QPushButton::clicked, this,
            &QtWalletUI::onImportTokenClicked);
    ...
    m_usdtWalletCard->setTransactionHistory(
        "No transactions yet.<br><br>USDT (Tether) is a stablecoin pegged to "
        "the US Dollar. "
    ...
    m_usdcWalletCard->setTransactionHistory(
        "No transactions yet.<br><br>USDC (USD Coin) is a stablecoin pegged to "
        "the US Dollar. "
        "It operates on the Ethereum network as an ERC20 token.");
    ...
    m_daiWalletCard->setTransactionHistory(
        "No transactions yet.<br><br>DAI is a decentralized stablecoin pegged "
        "to the US Dollar. "
        "It operates on the Ethereum network as an ERC20 token.");
...
void QtWalletUI::setUserInfo(const QString &username, const QString &address) {
  ...
  UserSession *session =
      Auth::AuthManager::getInstance().getSession(m_sessionId.toStdString());
  if (session) {
...
void QtWalletUI::onSendBitcoinClicked() {
  ...
  QtSendDialog dialog(QtSendDialog::ChainType::BITCOIN, currentBalance,
                      m_currentBTCPrice, this);
...
void QtWalletUI::onReceiveBitcoinClicked() {
  ...
  QtReceiveDialog dialog(QtReceiveDialog::ChainType::BITCOIN, m_currentAddress,
                         this);
  dialog.exec();
...
void QtWalletUI::onSendLitecoinClicked() {
  ...
  if (!m_litecoinWallet) {
    QMessageBox::warning(
        this, "Litecoin Wallet Not Available",
  ...
  QtSendDialog dialog(QtSendDialog::ChainType::LITECOIN, currentBalance,
                      m_currentLTCPrice, this);
  if (dialog.exec() == QDialog::Accepted) {
    auto txDataOpt = dialog.getTransactionData();
    if (!txDataOpt.has_value()) {
      return;
    ...
    progressBox->setText(
        "Broadcasting transaction to the Litecoin network...\nPlease wait.");
    progressBox->setStandardButtons(QMessageBox::NoButton);
    progressBox->setModal(true);
    progressBox->show();
    QApplication::processEvents();
    ...
    try {
      ...
      auto result = m_litecoinWallet->SendFunds(
          fromAddresses, toAddress, txData.amountLitoshis, privateKeysMap,
          txData.estimatedFeeLitoshis);
      ...
      if (result.success) {
        QMessageBox::information(
            ...
            QString(
                ...
                .arg(m_litecoinWallet->ConvertLitoshisToLTC(
                         txData.amountLitoshis),
                     0, 'f', 8)
                .arg(m_litecoinWallet->ConvertLitoshisToLTC(result.total_fees),
                     0, 'f', 8));
...
void QtWalletUI::onReceiveLitecoinClicked() {
  ...
  if (m_litecoinAddress.isEmpty()) {
    QMessageBox::warning(this, "No Litecoin Address",
                         "Litecoin address not available. Please ensure your "
                         "wallet is set up correctly.");
    return;
  }

  QtReceiveDialog dialog(QtReceiveDialog::ChainType::LITECOIN,
                         m_litecoinAddress, this);
  dialog.exec();
...
void QtWalletUI::onReceiveEthereumClicked() {
  ...
  if (m_ethereumAddress.isEmpty()) {
    QMessageBox::warning(this, "No Ethereum Address",
                         "Ethereum address not available. Please ensure your "
                         "wallet is set up correctly.");
    return;
  }

  QtReceiveDialog dialog(QtReceiveDialog::ChainType::ETHEREUM,
                         m_ethereumAddress, this);
  dialog.exec();
...
void QtWalletUI::onSendEthereumClicked() {
  ...
  if (!m_ethereumWallet) {
    QMessageBox::warning(
        this, "Ethereum Wallet Not Available",
  ...
  QtSendDialog dialog(QtSendDialog::ChainType::ETHEREUM, currentBalance,
                      m_currentETHPrice, this);
  if (dialog.exec() == QDialog::Accepted) {
    auto txDataOpt = dialog.getTransactionData();
    if (!txDataOpt.has_value()) {
      return;
    ...
    // Show confirmation
    QString confirmMsg = QString("You are about to send %1 ETH to:\n%2\n\n"
                                 "Gas Price: %3 Gwei\n"
                                 "Gas Limit: %4\n"
                                 "Total Cost: %5 ETH\n\n"
    ...
    int response =
        QMessageBox::question(this, "Confirm Ethereum Transaction", confirmMsg,
                              QMessageBox::Yes | QMessageBox::No);

    if (response == QMessageBox::Yes) {
      QMessageBox *progressBox = new QMessageBox(this);
      progressBox->setAttribute(
          Qt::WA_DeleteOnClose); // Ensure deletion on close
      ...
      try {
        ...
        auto result = m_ethereumWallet->SendFunds(
            ...
            txData.recipientAddress.toStdString(), txData.amountETH,
            privateKeyHex.toStdString(), txData.gasPriceGwei.toStdString(),
            txData.gasLimit);
        ...
        if (result.success) {
          QMessageBox::information(
              ...
              QString("Transaction sent successfully!\n\n"
                  ...
                  .arg(QString::number(
                      result.total_cost_eth * m_currentETHPrice, 'f', 2)));
...
void QtWalletUI::onSendUSDTClicked() {
  ...
  if (m_ethereumAddress.isEmpty()) {
    QMessageBox::warning(
        ...
        "Ethereum address not available. USDT is an ERC20 token on "
        "Ethereum.\n\n"
...
void QtWalletUI::onReceiveUSDTClicked() {
  if (m_ethereumAddress.isEmpty()) {
    QMessageBox::warning(
        this, "No Ethereum Address",
        "Ethereum address not available. USDT uses the same address as "
        "Ethereum.\n\n"
        "Please ensure your Ethereum wallet is set up correctly.");
    return;
  }
...
void QtWalletUI::onSendUSDCClicked() {
  if (m_ethereumAddress.isEmpty()) {
    QMessageBox::warning(
        this, "No Ethereum Address",
        "Ethereum address not available. USDC is an ERC20 token on "
        "Ethereum.\n\n"
        "Please ensure your Ethereum wallet is set up correctly.");
    return;
  }
  ...
  QMessageBox::information(
      ...
      "USDC (USD Coin) is an ERC20 token that operates on the Ethereum "
      "network. "
      "You will need ETH in your wallet to pay for gas fees.\n\n"
...
void QtWalletUI::onReceiveUSDCClicked() {
  if (m_ethereumAddress.isEmpty()) {
    QMessageBox::warning(
        this, "No Ethereum Address",
        "Ethereum address not available. USDC uses the same address as "
        "Ethereum.\n\n"
        "Please ensure your Ethereum wallet is set up correctly.");
    return;
  }
...
void QtWalletUI::onReceiveDAIClicked() {
  if (m_ethereumAddress.isEmpty()) {
    QMessageBox::warning(
        this, "No Ethereum Address",
        "Ethereum address not available. DAI uses the same address as "
        "Ethereum.\n\n"
        "Please ensure your Ethereum wallet is set up correctly.");
    return;
  }
...
void QtWalletUI::onImportTokenClicked() {
  ...
  if (tokenAddress.isEmpty() || tokenAddress.length() != 42 ||
      !tokenAddress.startsWith("0x")) {
    QMessageBox::warning(this, "Invalid Address",
                         "The contract address format is invalid.\n\n"
                         "Please enter a valid Ethereum address (42 characters "
                         "starting with 0x).");
    return;
  }
...
int QtWalletUI::getEthereumWalletId() {
  ...
  auto userWalletsResult =
      m_walletRepository->getWalletsByUserId(m_currentUserId);
...
void QtWalletUI::setupTokenManagement() {
  ...
  connect(m_tokenListWidget, &QtTokenListWidget::deleteTokenClicked, this,
          &QtWalletUI::onTokenDeleted);
...
void QtWalletUI::onTokenDeleted(const QString &contractAddress) {
  ...
  auto tokenResult = m_tokenRepository->getToken(ethereumWalletId,
                                                 contractAddress.toStdString());
  QString tokenName = tokenResult.success
                          ? QString::fromStdString(tokenResult.data.symbol)
                          : "this token";
  ...
  auto deleteResult = m_tokenRepository->deleteToken(
      ethereumWalletId, contractAddress.toStdString());
...
void QtWalletUI::updateStyles() {
  ...
  if (!m_themeManager || !m_headerSection || !m_headerTitle ||
      !m_balanceLabel) {
    return;
  }
    ...
    QString balanceTitleStyle = QString(R"(
        QLabel {
            ...
            background-color: transparent;
                                    ...
                                    .arg(m_themeManager->dimmedTextColor()
                                             .name()) // Theme-aware dimmed text
  ...
  if (m_toggleBalanceButton) {
    QString toggleButtonStyle =
        ...
        QPushButton:hover {
            ...
            .arg(m_themeManager->secondaryColor()
                     .name()) // Theme-aware hover background
...
void QtWalletUI::resizeEvent(QResizeEvent *event) {
  ...
  // Add safety checks before calling layout functions to prevent access
  // violations
...
void QtWalletUI::updateScrollAreaWidth() {
  ...
  if (windowWidth <= 768) {
    ...
    int targetWidth = static_cast<int>(
        availableWidth * 0.85); // Increased from 0.85 for better utilization
    ...
    int availableWidth = windowWidth - SIDEBAR_WIDTH;
    int targetWidth =
        static_cast<int>(availableWidth * 0.80); // Increased from 0.80
    m_scrollArea->setMaximumWidth(targetWidth);
    ...
    int availableWidth = windowWidth - SIDEBAR_WIDTH;
    int targetWidth =
        static_cast<int>(availableWidth * 0.75); // Increased from 0.70
    m_scrollArea->setMaximumWidth(targetWidth);
    ...
    int availableWidth = windowWidth - SIDEBAR_WIDTH;
    int targetWidth =
        static_cast<int>(availableWidth * 0.70); // Increased from 0.65
    m_scrollArea->setMaximumWidth(targetWidth);
    ...
    int availableWidth = windowWidth - SIDEBAR_WIDTH;
    int targetWidth =
        static_cast<int>(availableWidth * 0.65); // Increased from 0.55
    m_scrollArea->setMaximumWidth(targetWidth);
    ...
    int availableWidth = windowWidth - SIDEBAR_WIDTH;
    int targetWidth =
        static_cast<int>(availableWidth * 0.68); // Increased from 0.60
    m_scrollArea->setMaximumWidth(targetWidth);
    m_scrollArea->setMinimumWidth(qMax(1200, targetWidth));
    useFullWidth = false;
  }
  ...
  if (useFullWidth) {
    ...
    m_leftSpacer->changeSize(0, 0, QSizePolicy::Expanding,
                             QSizePolicy::Minimum);
    m_rightSpacer->changeSize(0, 0, QSizePolicy::Expanding,
                              QSizePolicy::Minimum);
  }
...
void QtWalletUI::updateResponsiveLayout() {
  ...
  if (windowWidth <= 480) {
    ...
    topMargin =
        20; // Reduced from 40 - floating button only needs minimal space
  ...
  m_mainLayout->setContentsMargins(leftMargin, topMargin, rightMargin,
                                   bottomMargin);
  m_contentLayout->setContentsMargins(contentMargin, contentMargin,
                                      contentMargin, contentMargin);
  ...
  // Adjust header and card sizing for better proportions - more refined padding
  // with safety checks
  if (m_headerSection) {
    ...
    if (headerVerticalPadding >= 0 && headerVerticalPadding <= 100) {
      m_headerSection->setContentsMargins(0, headerVerticalPadding, 0,
...
void QtWalletUI::updateUSDBalance() {
  ...
  double btcPriceToUse =
      (m_currentBTCPrice > 0.0) ? m_currentBTCPrice : 43000.0;
  ...
  double totalUsdBalance =
      btcUsdValue + ltcUsdValue + ethUsdValue + tokensUsdValue;
...
void QtWalletUI::sendRealTransaction(const QString &recipientAddress,
  ...
  auto seedResult = m_walletRepository->retrieveDecryptedSeed(
      m_currentUserId, password.toStdString());
  if (!seedResult.success) {
    throw std::runtime_error("Failed to decrypt seed: " +
  ...
  // Step 3: Derive the private key for the current address (testnet BIP44 path:
  // m/44'/1'/0'/0/0)
  Crypto::BIP32ExtendedKey addressKey;
  if (!Crypto::BIP44_DeriveAddressKey(masterKey, 0, false, 0, addressKey,
                                      true)) {
  ...
  // Convert WIF to raw private key bytes (WIF is base58 encoded, we need raw
  // bytes) For simplicity, we'll use the key directly from the extended key
  std::vector<uint8_t> privateKeyBytes(addressKey.key.begin(),
                                       addressKey.key.end());
  privateKeysMap[fromAddress] = privateKeyBytes;
  ...
  WalletAPI::SendTransactionResult result = m_wallet->SendFunds(
      fromAddresses, toAddress, amountSatoshis, privateKeysMap, feeSatoshis);
  ...
  QMessageBox::information(
      ...
      QString("Transaction broadcast successfully!\n\n"
          ...
          .arg(static_cast<double>(result.total_fees) / 100000000.0, 0, 'f',
               8));
...
std::vector<uint8_t>
QtWalletUI::derivePrivateKeyForAddress(const QString &address,
                                       const QString &password) {
  if (!m_walletRepository || m_currentUserId < 0) {
...
QString QtWalletUI::formatBitcoinTransactionHistory(
    const std::vector<std::string> &txHashes) {
  if (txHashes.empty()) {
    return "No transactions yet.<br><br>Send testnet Bitcoin to your address "
           "to see it appear "
           "here!";
  }
  ...
  historyHtml += QString("<b>Recent Transactions</b> (%1 total)<br><br>")
                     .arg(txHashes.size());
  ...
  for (const auto &txHash : txHashes) {
    ...
    historyHtml +=
        QString("<div style='margin-bottom: 12px; padding: 8px; background: "
                "rgba(128,128,128,0.1); "
                "border-radius: 4px;'>"
                "<b>TX:</b> <span style='font-family: monospace;'>%1</span><br>"
                "<span style='font-size: 10px; color: #666;'>Tap to view on "
                "block explorer</span>"
                "</div>")
            .arg(shortHash);
  ...
  if (txHashes.size() > 5) {
    historyHtml += QString("<br><i>... and %1 more transactions</i>")
                       .arg(txHashes.size() - 5);
  }
...
QString QtWalletUI::formatEthereumTransactionHistory(
    const std::vector<EthereumService::Transaction> &txs,
    const std::string &userAddress) {
  if (txs.empty()) {
    return "No transactions yet.<br><br>Send Ethereum to your address to see "
           "it appear here!";
  }
  ...
  historyHtml += "<div style='font-size: 12px;'>";
  historyHtml +=
      QString("<b>Recent Transactions</b> (%1 total)<br><br>").arg(txs.size());
  ...
  if (txs.size() > 5) {
    historyHtml +=
        QString("<br><i>... and %1 more transactions</i>").arg(txs.size() - 5);
  }

  historyHtml += "</div>";
  return historyHtml;
}
void QtWalletUI::setSessionId(const QString &sessionId) {
...
double QtWalletUI::calculateTotalTokenUSDValue() {
  ...
  // Since we can't directly access private members, we'll use the data from the
  // database and calculate USD values based on token prices
  ...
  // For now, we'll use a simplified approach since token USD values aren't
  // stored In a real implementation, you would fetch token prices from an API
    ...
    if (m_ethereumWallet && !m_ethereumAddress.isEmpty()) {
      ...
      if (balanceOpt.has_value()) {
        ...
        if (m_priceFetcher) {
          ...
          auto priceDataOpt = m_priceFetcher->GetCryptoPrice(
              symbolUpper.toLower().toStdString());
          if (priceDataOpt.has_value()) {
        ...
        if (tokenPriceUSD <= 0.0) {
          if (symbolUpper == "USDT" || symbolUpper == "USDC" ||
            ...
            tokenPriceUSD = m_currentBTCPrice > 0.0
                                ? m_currentBTCPrice
                                : 43000.0; // Wrapped BTC follows BTC price
          } else if (symbolUpper == "WETH") {
            tokenPriceUSD = m_currentETHPrice > 0.0
                                ? m_currentETHPrice
                                : 2500.0; // Wrapped ETH follows ETH price
          }
        }
