...
QtWalletUI::QtWalletUI(QWidget* parent)
    ...
    connect(m_balanceFetchWatcher, &QFutureWatcher<BalanceFetchResult>::finished, this,
            &QtWalletUI::onBalanceFetchFinished);
...
void QtWalletUI::onSendBitcoinClicked() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        // SECURITY: Wipe password from transaction data after use
        txData.password.fill(QChar('\0'));
        txData.password.clear();
...
void QtWalletUI::onSendLitecoinClicked() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        try {
            ...
            auto result =
                m_litecoinWallet->SendFunds(fromAddresses, toAddress, txData.amountLitoshis,
            ...
            if (result.success) {
                QMessageBox::information(
                    ...
                    QString("Transaction sent successfully!\n\n"
                        ...
                        .arg(m_litecoinWallet->ConvertLitoshisToLTC(txData.amountLitoshis), 0, 'f',
                             8)
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (response == QMessageBox::Yes) {
            ...
            try {
                ...
                privateKeyHex.fill(QChar('\0'));
                privateKeyHex.clear();
                Crypto::SecureWipeVector(privateKeyBytes);
                // SECURITY: Wipe password from transaction data
                txData.password.fill(QChar('\0'));
                txData.password.clear();
...
void QtWalletUI::onImportTokenClicked() {
    ...
    if (m_currentUserId < 0) {
        QMessageBox::warning(this, "Not Logged In", "Please log in to import tokens.");
        return;
    }
...
void QtWalletUI::updateTokenBalances() {
    ...
    auto watcher = new QFutureWatcher<std::vector<TokenBalanceUpdate>>(this);
    connect(watcher, &QFutureWatcher<std::vector<TokenBalanceUpdate>>::finished, this,
            [this, watcher]() {
                auto updates = watcher->result();
                for (const auto& update : updates) {
                    QString contractAddress = QString::fromStdString(update.contractAddress);
                    if (update.success) {
...
void QtWalletUI::onTokenDeleted(const QString& contractAddress) {
    ...
    QString tokenName =
        tokenResult.success ? QString::fromStdString(tokenResult.data.symbol) : "this token";
    ...
    auto deleteResult =
        m_tokenRepository->deleteToken(ethereumWalletId, contractAddress.toStdString());
    if (deleteResult.success) {
        ...
        QMessageBox::warning(this, "Error", "Failed to remove token. Please try again.");
    }
}
...
void QtWalletUI::setEthereumWallet(WalletAPI::EthereumWallet* ethWallet) {
    ...
    if (m_ethereumWallet && m_tokenRepository && m_ethereumWalletCard && !m_tokenListWidget) {
...
void QtWalletUI::fetchRealBalance() {
    ...
    auto future = QtConcurrent::run([wallet, litecoinWallet, ethereumWallet, tokenRepository,
                                     address, litecoinAddress, ethereumAddress, ethereumWalletId,
        ...
        if (wallet && !address.empty()) {
            try {
                ...
                result.errorMessage = "Failed to fetch Bitcoin balance: " + std::string(e.what());
            }
        }

        if (litecoinWallet && !litecoinAddress.empty()) {
            try {
                uint64_t balanceLitoshis = litecoinWallet->GetBalance(litecoinAddress);
                result.ltcBalance = litecoinWallet->ConvertLitoshisToLTC(balanceLitoshis);
                result.ltcTransactions = litecoinWallet->GetTransactionHistory(litecoinAddress, 10);
                result.ltcSuccess = true;
            } catch (const std::exception& e) {
                if (result.errorMessage.empty()) {
                    result.errorMessage =
                        "Failed to fetch Litecoin balance: " + std::string(e.what());
                }
            }
        }
        ...
        if (ethereumWallet && !ethereumAddress.empty()) {
            try {
                ...
                result.ethTransactions = ethereumWallet->GetTransactionHistory(ethereumAddress, 10);
                result.ethSuccess = true;
            } catch (const std::exception& e) {
                if (result.errorMessage.empty()) {
                    result.errorMessage =
                        "Failed to fetch Ethereum balance: " + std::string(e.what());
                }
            }
        }
        ...
        if (tokenRepository && ethereumWallet && !ethereumAddress.empty() &&
            ...
            if (tokensResult.success) {
                ...
                for (const auto& token : tokensResult.data) {
                    ...
                    std::transform(symbolLower.begin(), symbolLower.end(), symbolLower.begin(),
                                   [](unsigned char c) {
                                       return std::tolower(c);
                                   });

                    if (auto priceData = priceFetcher.GetCryptoPrice(symbolLower);
                        priceData.has_value()) {
                    ...
                    if (tokenPriceUsd <= 0.0) {
                        if (symbolUpper == "USDT" || symbolUpper == "USDC" ||
                            ...
                            tokenPriceUsd = currentBtcPrice > 0.0 ? currentBtcPrice : 43000.0;
                        } else if (symbolUpper == "WETH") {
                            tokenPriceUsd = currentEthPrice > 0.0 ? currentEthPrice : 2500.0;
                        }
                    }
...
void QtWalletUI::onBalanceFetchFinished() {
    ...
    if (result.ethSuccess) {
        ...
        if (m_ethereumWalletCard) {
            m_ethereumWalletCard->setBalance(QString("%1 ETH").arg(m_realBalanceETH, 0, 'f', 8));
            m_ethereumWalletCard->setTransactionHistory(formatEthereumTransactionHistory(
                result.ethTransactions, m_ethereumAddress.toStdString()));
        }
    }
    ...
    if (m_tokenListWidget && !result.tokenBalances.empty()) {
        for (const auto& update : result.tokenBalances) {
            QString contractAddress = QString::fromStdString(update.contractAddress);
            if (update.success) {
                m_tokenListWidget->updateTokenBalance(contractAddress,
                                                      QString::fromStdString(update.balanceText));
            } else {
                m_tokenListWidget->updateTokenBalance(contractAddress, "Error", "");
            }
        }
...
void QtWalletUI::setTokenRepository(Repository::TokenRepository* tokenRepo) {
    ...
    if (m_tokenRepository && m_ethereumWallet && m_ethereumWalletCard && !m_tokenListWidget) {
        setupTokenManagement();
    }
}
...
double QtWalletUI::calculateTotalTokenUSDValue() {
    ...
    for (const auto& token : tokensResult.data) {
        ...
        if (m_ethereumWallet && !m_ethereumAddress.isEmpty()) {
            auto balanceOpt = m_ethereumWallet->GetTokenBalance(m_ethereumAddress.toStdString(),
            ...
            if (balanceOpt.has_value()) {
                ...
                if (m_priceFetcher) {
                    ...
                    auto priceDataOpt =
                        m_priceFetcher->GetCryptoPrice(symbolUpper.toLower().toStdString());
                    if (priceDataOpt.has_value()) {
