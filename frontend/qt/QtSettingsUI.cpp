...
QtSettingsUI::QtSettingsUI(QWidget* parent)
    : QWidget(parent),
      ...
      m_btcProviderStatusLabel(nullptr),
...
void QtSettingsUI::setRepositories(Repository::UserRepository* userRepository,
...
void QtSettingsUI::setupUI() {
    ...
    m_themeSelector->addItem("Dark - Purple", static_cast<int>(ThemeType::CRYPTO_DARK));
    m_themeSelector->addItem("Light - Purple", static_cast<int>(ThemeType::CRYPTO_LIGHT));
    ...
    m_walletPlaceholder =
        new QLabel("Configure node providers and hardware wallets for this device.", walletGroup);
    ...
    QPushButton* configureProviderButton =
        new QPushButton("Configure Node Provider", providerSection);
    ...
    connect(configureProviderButton, &QPushButton::clicked, this, [this]() {
        ...
        titleLabel->setStyleSheet(QString("font-size: 22px; font-weight: 700; color: %1;")
                                      .arg(m_themeManager->textColor().name()));
        cardLayout->addWidget(titleLabel);

        QLabel* descLabel =
            new QLabel("Connect to BlockCypher's public API or your own Bitcoin Core node.", card);
        descLabel->setWordWrap(true);
        descLabel->setStyleSheet(
            QString("font-size: 14px; color: %1;").arg(m_themeManager->subtitleColor().name()));
        ...
        std::vector<std::string> keys = {
            kSettingsProviderTypeKey, kSettingsRpcUrlKey,           kSettingsRpcUsernameKey,
            kSettingsRpcPasswordKey,  kSettingsRpcAllowInsecureKey, kSettingsProviderFallbackKey};
        ...
        connect(providerSelector, QOverload<int>::of(&QComboBox::currentIndexChanged),
                updateVisibility);
        ...
        connect(testBtn, &QPushButton::clicked, [=]() {
            ...
            QFutureWatcher<std::pair<bool, std::string>>* watcher =
                new QFutureWatcher<std::pair<bool, std::string>>(dialog);
                ...
                try {
                    ...
                    if (!provider) {
                        return std::make_pair(
                            false, std::string("Failed to create provider configuration. Check "
                                               "settings (e.g. insecure HTTP)."));
                    }
                    return provider->testConnection();
                } catch (const std::exception& e) {
                    return std::make_pair(
                        false, std::string("Exception during connection test: ") + e.what());
                } catch (...) {
                    return std::make_pair(false,
                                          std::string("Unknown exception during connection test"));
                }
            });
    ...
    QPushButton* configureHardwareButton =
        new QPushButton("Configure Hardware Wallet", hardwareSection);
    ...
    // These will be managed by the dialog now, but we keep the members for the existing slots if
    // needed or we refactor the slots. For now, init to nullptr.
    m_hardwareWalletSelector = nullptr;
    m_hardwareDerivationPathEdit = nullptr;
    m_hardwareUseTestnetCheck = nullptr;
    m_hardwareDetectButton = nullptr;
    ...
    connect(configureHardwareButton, &QPushButton::clicked, this, [this]() {
        if (!m_settingsRepository || m_currentUserId <= 0) {
            QMessageBox::warning(this, "Not Signed In",
                                 "Please sign in to configure hardware wallet.");
            return;
        }
        ...
        connect(importBtn, &QPushButton::clicked, [=]() {
            ...
            if (wallets.success && !wallets.data.empty()) {
                auto updateRes = m_walletRepository->updateWallet(
                    wallets.data.front().id, std::nullopt, path.toStdString(), xpub.toStdString());
                if (updateRes.success) {
                    m_hardwareStatusLabel->setText("Hardware wallet configured.");
                    dialog->accept();
                    QMessageBox::information(this, "Success",
                                             "Hardware wallet imported successfully.");
                } else {
...
void QtSettingsUI::updateStyles() {
    ...
    QString checkboxStyle =
        QString(
            "QCheckBox { color: %1; } QCheckBox::indicator { border: 1px solid %2; width: 14px; "
            "height: 14px; } QCheckBox::indicator:checked { background-color: %3; }")
            .arg(textColor)
            .arg(secondaryColor)
            .arg(accentColor);
...
void QtSettingsUI::loadAdvancedSettings() {
    if (!m_btcProviderSelector || !m_btcRpcUrlEdit || !m_btcRpcUsernameEdit ||
        !m_btcRpcPasswordEdit || !m_btcAllowInsecureCheck || !m_btcEnableFallbackCheck ||
        !m_hardwareXpubDisplay || !m_btcProviderStatusLabel || !m_hardwareStatusLabel) {
        return;
    }
    ...
    if (!m_settingsRepository || m_currentUserId <= 0) {
        ...
        m_btcProviderStatusLabel->setText("Sign in to save provider settings.");
    ...
    auto settingsResult = m_settingsRepository->getUserSettings(m_currentUserId, keys);
    ...
    m_btcRpcUrlEdit->setText(urlIt != settings.end() ? QString::fromStdString(urlIt->second)
                                                     : QString());

    auto userIt = settings.find(kSettingsRpcUsernameKey);
    m_btcRpcUsernameEdit->setText(userIt != settings.end() ? QString::fromStdString(userIt->second)
                                                           : QString());

    auto passIt = settings.find(kSettingsRpcPasswordKey);
    m_btcRpcPasswordEdit->setText(passIt != settings.end() ? QString::fromStdString(passIt->second)
                                                           : QString());
    ...
    m_btcProviderStatusLabel->setText("Provider settings loaded for this device.");

    if (m_walletRepository) {
        auto walletResult = m_walletRepository->getWalletsByType(m_currentUserId, "bitcoin", true);
        if (walletResult.success && !walletResult.data.empty()) {
            ...
            if (wallet.extendedPublicKey.has_value()) {
                m_hardwareXpubDisplay->setText(QString::fromStdString(*wallet.extendedPublicKey));
...
void QtSettingsUI::updateHardwareWalletStatus(const QString& message, bool success) {
    if (m_hardwareStatusLabel) {
...
void QtSettingsUI::onSaveAdvancedSettings() {
    if (!m_settingsRepository || m_currentUserId <= 0) {
        QMessageBox::warning(this, "Not Signed In", "Please sign in to save settings.");
        return;
    }

    QString providerType = m_btcProviderSelector->currentData().toString().trimmed();
    ...
    auto saveResult = m_settingsRepository->setUserSetting(
        m_currentUserId, kSettingsProviderTypeKey, providerType.toStdString());
    if (!saveResult.success) {
        QMessageBox::warning(this, "Save Failed", "Failed to save provider type.");
        return;
    }

    m_settingsRepository->setUserSetting(m_currentUserId, kSettingsRpcUrlKey, rpcUrl.toStdString());
    m_settingsRepository->setUserSetting(m_currentUserId, kSettingsRpcUsernameKey,
    ...
    m_btcProviderStatusLabel->setText("Provider settings saved on this device.");

    emit bitcoinProviderSettingsChanged(providerType, rpcUrl, rpcUsername, rpcPassword,
                                        allowInsecure, allowFallback);
}

void QtSettingsUI::onTestRpcConnection() {
    QString rpcUrl = m_btcRpcUrlEdit->text().trimmed();
    if (rpcUrl.isEmpty()) {
        QMessageBox::warning(this, "Missing RPC URL", "Please enter the RPC URL to test.");
        return;
    }

    if (rpcUrl.startsWith("http://") && !m_btcAllowInsecureCheck->isChecked()) {
        QMessageBox::warning(this, "Insecure RPC URL", "Enable HTTP for local node or use HTTPS.");
        return;
    }
    ...
    if (!username.isEmpty()) {
        QByteArray credentials = QString("%1:%2").arg(username, password).toUtf8().toBase64();
    ...
    QNetworkReply* reply = manager.post(rpcNetworkRequest, QJsonDocument(payload).toJson());
    ...
    if (!timeoutTimer.isActive()) {
        reply->abort();
        reply->deleteLater();
        QMessageBox::warning(this, "RPC Timeout", "RPC request timed out after 10 seconds.");
        return;
    }
    ...
    if (reply->error() != QNetworkReply::NoError) {
        ...
        reply->deleteLater();
        QMessageBox::warning(this, "RPC Error", "RPC request failed: " + errorMessage);
        return;
    }
    ...
    if (!responseDoc.isObject() || !responseDoc.object().contains("result")) {
        QMessageBox::warning(this, "RPC Error", "RPC response missing expected data.");
        return;
    }

    QMessageBox::information(this, "RPC Connected", "Successfully connected to your RPC node.");
    if (m_btcProviderStatusLabel) {
...
void QtSettingsUI::onDetectHardwareWallets() {
    ...
    if (output.isEmpty()) {
        updateHardwareWalletStatus(
            errorOutput.isEmpty() ? "No hardware wallets detected." : errorOutput, false);
        return;
    }
    ...
    updateHardwareWalletStatus("Hardware wallet detected. Select to import xpub.", true);
}
...
void QtSettingsUI::onImportHardwareXpub() {
    ...
    if (!deviceData.isValid()) {
        QMessageBox::warning(this, "No Device", "Please detect and select a hardware wallet.");
        return;
    }
    ...
    if (derivationPath.isEmpty()) {
        QMessageBox::warning(this, "Missing Path", "Please enter a derivation path.");
        return;
    }
    ...
    if (output.isEmpty()) {
        updateHardwareWalletStatus(errorOutput.isEmpty() ? "Failed to retrieve xpub." : errorOutput,
                                   false);
        return;
    }
    ...
    auto walletResult = m_walletRepository->getWalletsByType(m_currentUserId, "bitcoin", true);
    if (!walletResult.success || walletResult.data.empty()) {
        QMessageBox::warning(this, "Wallet Missing", "No Bitcoin wallet found to store the xpub.");
        return;
    }
    ...
    if (!updateResult.success) {
        QMessageBox::warning(this, "Update Failed", "Failed to store hardware wallet xpub.");
        return;
    }

    m_hardwareXpubDisplay->setText(xpub);
    updateHardwareWalletStatus("Hardware wallet xpub imported successfully.", true);
}
...
void QtSettingsUI::onDisable2FAClicked() {
    ...
    if (ret == QMessageBox::Yes) {
        ...
        QLabel* backupLabel = new QLabel(
            "<small>Lost your authenticator? <a href='backup'>Use a "
            "backup code</a></small>",
            &disableDialog);
